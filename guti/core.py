...
def get_voxel_mask(resolution: float = 1) -> np.ndarray:
    ...
    nx = int(2 * SCALP_RADIUS / resolution)
    ny = int(2 * SCALP_RADIUS / resolution)
    nz = int(SCALP_RADIUS / resolution)
    mask = np.zeros((nx, ny, nz))

    # Create coordinate grids
    x = np.linspace(-SCALP_RADIUS, SCALP_RADIUS, nx)
    y = np.linspace(-SCALP_RADIUS, SCALP_RADIUS, ny)
