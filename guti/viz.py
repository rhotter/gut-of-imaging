...
def visualize_brain_model(sources=None, sensors=None, field=None, resolution=1, alpha=0.2, figsize=(10, 8)):
    ...
    if field is None:
        ...
        if field.shape != mask.shape:
            raise ValueError(f"Field shape {field.shape} does not match mask shape {mask.shape}")
...
def plot_slice(field, slice_dim='z', slice_index=None, mask=None, resolution=1, figsize=(8, 6)):
    """
    ...
    # Plot the slice
    im = ax.imshow(masked_data.T, origin='lower', extent=extent, cmap='viridis', aspect='equal')
